...
//export k8s_client_corev1_rcs_list
func k8s_client_corev1_rcs_list(clientsetKey C.uintptr_t, ns *C.char, optsBytes unsafe.Pointer, optsSize C.int, oBytes *unsafe.Pointer, oSize *C.int) *C.char {
	listOptions := metav1.ListOptions{}
	listOptions.Unmarshal(no_copy_slice_from_c_array(optsBytes, optsSize))
	objList, err := clientsetMap[clientsetKey].CoreV1().ReplicationControllers(C.GoString(ns)).List(context.Background(), listOptions)
	if err != nil {
...
type rcWatchHandlerFunc struct {
	callbackFn      C.uintptr_t
	callbackContext C.uintptr_t
}

func (h *rcWatchHandlerFunc) HandleEvent(eventType int, o interface{}) error {
	obj, ok := o.(*apiv1.ReplicationController)
	if ok {
...
//export k8s_client_corev1_rcs_watch
func k8s_client_corev1_rcs_watch(clientsetKey C.uintptr_t, ns *C.char, optsBytes unsafe.Pointer, optsSize C.int,
	callbackFn C.uintptr_t, callbackContext C.uintptr_t) *C.char {
	listOptions := metav1.ListOptions{}
	listOptions.Unmarshal(no_copy_slice_from_c_array(optsBytes, optsSize))
	listOptions.ResourceVersion = "0"
	watch, err := clientsetMap[clientsetKey].CoreV1().ReplicationControllers(C.GoString(ns)).Watch(context.Background(), listOptions)
	if err != nil {
		return C.CString(err.Error())
	}
	...
	go watchHandler(watch, reflect.TypeOf(&apiv1.ReplicationController{}), &rcWatchHandlerFunc{callbackFn, callbackContext}, stopCh)

	return nil
}

//export k8s_client_corev1_rcs_stop_watch
func k8s_client_corev1_rcs_stop_watch(watchH C.uintptr_t) C.int {
	watchMu.Lock()
	defer watchMu.Unlock()
